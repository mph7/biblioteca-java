...
public class Emprestimo {
  ...
  public Emprestimo(
      Aluno aluno, String titulo, Date dataEmprestimo, String autor, int numeroPaginas) {
    this.aluno = aluno;
    this.titulo = titulo;
  ...
  public void devolver(Emprestimo emprestimo, Date dataDevolucao) {
    long diferenca =
        ((((dataDevolucao.getTime() - dataEmprestimo.getTime()) / 1000) / 60) / 60) / 24;

    if (diferenca < 3) {
      System.out.println("Devolução concluída com Sucesso!");
    } else {
      System.out.println(
          "Para devolver o livro, você precisa pagar uma multa "
              + "no valor de: "
              + (diferenca - 3) * 2);
